...
using it.unical.mat.embasp.specializations.dlvhex.desktop;
...
{
    [TestClass()]
    public class DLVHEXDesktopServiceTest
    {
        ...
        public string GetPath()
        {
            ...
            if (OS.IndexOf("win", StringComparison.CurrentCultureIgnoreCase) >= 0)
                path.Append("win" + Path.DirectorySeparatorChar + "dlvhex2.exe");
            else if (OS.IndexOf("mac", StringComparison.CurrentCultureIgnoreCase) >= 0)
                path.Append("mac" + Path.DirectorySeparatorChar + "dlvhex2_mac");
        ...
        public void SudokuTest()
        {
            try
            {
                Handler handler = new DesktopHandler(new DLVHEXDesktopService(GetPath()));
